...
create policy "favorites_update_own" on public.favorites for update using (user_id = auth.uid()) with check (user_id = auth.uid());
create policy "favorites_delete_own" on public.favorites for delete using (user_id = auth.uid());

create policy "sources_select_own" on public.sources for select using (user_id = auth.uid());
create policy "sources_insert_own" on public.sources for insert with check (user_id = auth.uid());
create policy "sources_update_own" on public.sources for update using (user_id = auth.uid()) with check (user_id = auth.uid());
create policy "sources_delete_own" on public.sources for delete using (user_id = auth.uid());
