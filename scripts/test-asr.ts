...
function extractTranscriptText(payload: AsrTranscriptPayload) {
  let text = "";
...
async function main() {
  ...
  const submitJson = (await submitRes.json()) as AsrTaskPayload;
  ...
  let attempt = 0;
  let finalResult: AsrTaskPayload | null = null;
  ...
  while (attempt < MAX_ATTEMPTS) {
    ...
    const pollJson = (await pollRes.json()) as AsrTaskPayload;
  ...
  const transcriptionUrl = finalResult.output?.result?.transcription_url;

  if (transcriptionUrl) {
    console.log(`  transcription_url: ${transcriptionUrl.slice(0, 80)}...`);
    const transRes = await fetch(transcriptionUrl);
    const transJson = (await transRes.json()) as AsrTranscriptPayload;

    // 提取文本
    let text = extractTranscriptText(transJson);
