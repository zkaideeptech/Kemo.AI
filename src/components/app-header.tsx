    ...
    <header className="sticky top-0 z-40 w-full bg-card/40 backdrop-blur-2xl border-b border-border shadow-[0_4px_30px_rgba(0,0,0,0.1)]">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6">
        <div className="flex items-center gap-8">
          <Link href={`/${locale}`} className="flex items-center gap-2 text-xl font-bold tracking-tight hover:text-primary transition-colors duration-300">
            <KemoMark className="h-[22px] w-[22px] -mt-0.5" />
            <span>{t("appName")}</span>
          </Link>
          {hasSession && (
